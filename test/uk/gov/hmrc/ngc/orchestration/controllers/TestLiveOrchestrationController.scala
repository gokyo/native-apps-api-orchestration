...
class TestLiveOrchestrationController(
   appNameConfiguration: Configuration, auditConnector: AuditConnector, authConnector: AuthConnector,
   service: LiveOrchestrationService, actorSystem: ActorSystem, lifecycle: ApplicationLifecycle,
   reactiveMongo: ReactiveMongoComponent, serviceMax: Int, eventMax:Int, confLevel: Int , maxAgeForSuccess: Int,
   override val actorName: String ) extends
  LiveOrchestrationController(
    appNameConfiguration: Configuration, auditConnector: AuditConnector, authConnector: AuthConnector,
    service: LiveOrchestrationService, actorSystem, lifecycle, new TestProvider[ReactiveMongoComponent](reactiveMongo),
    serviceMax: Int, eventMax:Int, confLevel: Int , maxAgeForSuccess: Int ) {
